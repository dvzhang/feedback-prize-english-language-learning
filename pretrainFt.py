...
def make_loader(
    ...
    train_set, valid_set = data[data['kfold']
                                != fold], data[data['kfold'] == fold]
    ...
    def train(self, train_loader, valid_loader, epoch,
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            if batch_idx % self.log_interval == 0:
                ...
                ret = [
                    ('epoch: {:0>3} [{: >' + _s + '}/{} ({: >3.0f}%)]').format(epoch, count, len(
                        train_loader.sampler), 100 * count / len(train_loader.sampler)),
            ...
            if batch_idx % self.evaluate_interval == 0:
                ...
                if result_dict['val_loss'][-1] < result_dict['best_val_loss']:
                    print("{} epoch, best epoch was updated! valid_loss: {: >4.5f}".format(
...
def config(fold=0):
    ...
    model, tokenizer = make_model(
        model_name='./output/', num_labels=1)
...
[print("FOLD::", i, "Loss:: ", fold['best_val_loss'])
 for i, fold in enumerate(result_list)]
...
for fold in tqdm(range(5), total=5):
    ...
    train_loader, val_loader = make_loader(
        train, tokenizer, 250, 16, fold=fold)
